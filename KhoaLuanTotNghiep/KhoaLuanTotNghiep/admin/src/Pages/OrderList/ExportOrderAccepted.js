...
export const ExportOrderAccepted = ({ apiData, fileName }) => {
  ...
  React.useEffect(() => {
    const fetchData = () => {
      axios.get('https://localhost:5001/RealEstate/exportOrderAccepted').then(r => setData(r.data))
    }
    fetchData()
  }, [])
  const fileType =
  ...
  return (  
      <Button variant="warning"  style={{width: '20%', textAlign: 'center', lineHeight: '30px'}}  onClick={(e) => exportToCSV(apiData, fileName)}>Export Orders</Button>
  );
};