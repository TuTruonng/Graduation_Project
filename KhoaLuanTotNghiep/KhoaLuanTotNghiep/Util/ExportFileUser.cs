...
namespace KhoaLuanTotNghiep_BackEnd.Util
{
    ...
    {
        public static FileContentResult ExportExcel(ICollection<UserModel> users, string workSheetName, string fileName)
        {
            if (users.Count > 0)
            {
                using (var workbook = new XLWorkbook())
                {
                    ...
                    var currentRow = 1;
                    PropertyInfo[] properties = users.FirstOrDefault().GetType().GetProperties();//Lấy tên và giá trị các thuộc tính đầu tiên
                    ...
                    foreach (var model in users)
                    {
                        ...
                        for (int i = 0; i < propertyInfos.Length; i++)
                        {
                            try
                            {
                                worksheet.Cell(currentRow, i + 1).Value = propertyInfos[i].GetValue(model).ToString();
                            }
